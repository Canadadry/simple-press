...
func GetRouter(services Services) (http.HandlerFunc, error) {
	...
	r.Get("/admin/block", c.GetBlockList)
	r.Get("/admin/block/add", c.GetBlockAdd)
	r.Post("/admin/block/add", c.PostBlockAdd)
	r.Get("/admin/block/:path/edit", c.GetBlockEdit)
	r.Post("/admin/block/:path/edit", c.PostBlockEdit)
	r.Get("/admin/layout", c.GetLayoutList)
	r.Get("/admin/layout/add", c.GetLayoutAdd)
	r.Post("/admin/layout/add", c.PostLayoutAdd)
	r.Get("/admin/layout/:path/edit", c.GetLayoutEdit)
	r.Post("/admin/layout/:path/edit", c.PostLayoutEdit)
