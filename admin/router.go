...
func GetRouter(services Services) (http.HandlerFunc, error) {
	...
	r.Get("/admin/layouts", c.GetTemplateList)
	r.Get("/admin/layout/add", c.GetTemplateAdd)
	r.Post("/admin/layout/add", c.PostTemplateAdd)
	r.Get("/admin/layouts/:path/edit", c.GetTemplateEdit)
	r.Post("/admin/layouts/:path/edit", c.PostTemplateEdit)
